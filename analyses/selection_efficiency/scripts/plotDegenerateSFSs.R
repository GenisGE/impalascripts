...
outpng <- args[3]
winsfs <- F
if(length(args)>3) winsfs <- ifelse(args[4] == "winsfs", T, F)

load_sfs <- function(f, winsfs=F){

    if(winsfs){
        sfs <- scan(f, what=4, skip=1)
    }else{
        sfs <- scan(f, what=4)
    }
...
sfss <- lapply(files, load_sfs, winsfs=winsfs)
...
outpng <- gsub(".png", "_nofixedderived.png", outpng)
bitmap(outpng, h=6, w=6, res=300)
barplot(m[,-c(1,ncol(m))], beside=T, col=2:4, main=paste(p, "sfs in coding sites by degeneracy"), names.arg=1:(ncol(m)-2), xlab="Derived Allele Count")
legend("topright", fill=2:4, legend=rownames(m))
dev.off()
