...
rule findgraphs:
    input:
        ancient(rules.extractf2.output)
    output:
    ...
    threads: 80
    shell:
        "{R} {graphexplorer} --f2dir {input} --outprefix {params.outbase} --outpop {OUTGROUP} --usepops {USEPOPS} --nadmix {wildcards.nadmix} --nruns {START_PER_RUN} --threads {threads} > /dev/null"
